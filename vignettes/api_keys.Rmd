...
Data is downloaded from two platforms and each require an API key to be set. Keys for both [MfE](https://data.mfe.govt.nz/) and [NIWA](https://niwa.co.nz/) can be set by the `eeda_auth()` function; your API key shoulc be supplied as a character string to the  `key` arqument, by default `platform = "mfe"`.
...
```{r niwa key, eval = FALSE}
eeda_auth(key = key, platform = "niwa")
```
