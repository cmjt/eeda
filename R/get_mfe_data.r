...
mfe_data <- function(key){
    require(rvest)
    url <- paste("https://data.mfe.govt.nz/services;key=",key,"/wfs?service=WFS&request=GetCapabilities",sep = "")
    pg <- read_html(url)
    titles <- pg %>% html_nodes("title") %>% html_text()
    abstracts <- pg %>% html_nodes("abstract") %>% html_text()
    layerid <- pg %>% html_nodes("name") %>% html_text()
    res <- data.frame(Title = titles[-1], Abstract = abstracts[-1], Layerid = layerid)
...
#' Function to download a specified spatial layer from \url{http://data.mfe.govt.nz}
...
get_mfe_data <- function(id, key,plot = FALSE){
    ...
    urlp2 <- paste("/wfs?service=WFS&version=2.0.0&request=GetFeature&typeNames=",id,"&outputFormat=KML",sep = "")
